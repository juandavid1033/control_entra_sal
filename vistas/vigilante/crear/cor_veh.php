...
if (isset($_POST["validar_V"])) {
    ...
    $insertsql = $conex->prepare("INSERT INTO color (nom_color) VALUES (?)");
    $insertsql->execute([$color]);
    echo '<script>alert("Color creado exitosamente. Gracias.");</script>';
    echo '<script>window.location="./vehiculo.php"</script>';
    exit;
}
?>
                    ...
                    <div class="col-lg-20">
                        <div class="p-5">
                            <div class="text-center">
                                <h1 class="h4 mb-4">Crear Color</h1>
                            </div>
                            <form method="post" autocomplete="off" name="cli" enctype="multipart/form-data">
                                <div class="col-sm-20 mb-7 mb-sm-2">
                                    <label>Color</label>
                                    <input type="text" class="form-control form-control-lg form-control-user w-100" id="color" name="color" placeholder="Ingresa el color" required maxlength="16" onkeypress="return sololetras(event)">
                                    <small id="colorHelp" class="form-text text-danger"></small>
                                </div>
