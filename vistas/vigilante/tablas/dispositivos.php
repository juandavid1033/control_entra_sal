...
$sql1 = $conex->prepare("SELECT * FROM dispositivos 
...
$sql = $conex->prepare("SELECT COUNT(*) FROM dispositivos ORDER BY documento");
$sql->execute();
...
<a class="btn btn-primary" href="../index.php" style="margin: 3% 0 0 3%; position:absolute;">
    <i class="bi bi-chevron-left" style="padding:10px; color:#fff; font-size:15px; background-color:#0d6efd; border-radius:10px;"> REGRESAR</i>
</a>

<div class="container mt-4">
    <div class="d-flex justify-content-end mb-3">
        <a class="btn btn-success me-2" href="exceldispositivo.php">
            <i class="bi bi-file-earmark-excel" style="padding:10px; border-radius:10px; color:#fff; font-size:15px; background-color:#198754;"> EXCEL</i>
        </a>
        <a class="btn btn-danger" href="pdfdispositivo.php">
            <i class="bi bi-printer" style="padding:10px; border-radius:10px; color:#fff; font-size:15px; background-color:#E00000;"> IMPRIMIR</i>
        </a>
    </div>
                ...
                <tbody>
                    <?php foreach ($resultado1 as $usu): ?>
                        <tr>
                            ...
                            <td><?= $usu['serial'] ?></td>
                            <td><?= $usu['nom_marca'] ?></td>
                            <td><img src="../../../images/dispo/<?= $usu['imagen'] ?>" alt="Imagen del dispositivo" width="100"></td>
                            <td><?= $usu['nom_color'] ?></td>
                            <td><?= $usu['nom_dispositivo'] ?></td>
                            <td><?= $usu['nom_estado'] ?></td>
                            <td>
                                <form method="post" action="estado/estado.php">
                                    <input type="hidden" name="dispoacti" value="<?= $usu['serial'] ?>">
                                    <button type="submit" class="btn btn-success btn-sm">Activo</button>
                                </form>
                            </td>
                            <td>
                                <form method="post" action="estado/estado.php">
                                    <input type="hidden" name="inadispo" value="<?= $usu['serial'] ?>">
